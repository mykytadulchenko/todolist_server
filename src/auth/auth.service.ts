...
export class AuthService {
    ...
    async getTokens(id: string) {
        const tokens = await this.jwtRepository.createQueryBuilder()
                                            .select()
                                            .where('id = :id', { id })
                                            .getOne()
    ...
    generateTokens(payload) {
        return {
            ...
            refreshToken:  this.jwtService.sign(payload, { expiresIn: process.env.JWT_REFRESH_EXPIRE })
        }
    }
    ...
    async renewToken(token: string) {
        try {
            ...
            const { accessToken, refreshToken } = this.generateTokens({ id: payload.id, login: payload.login })
